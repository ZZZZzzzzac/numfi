...
class numfi(numfi_tmp): 
    ...
    def __fixed_arithmetic__(self, func, y):
        ...
        if name == 'add' or name == 'sub':            
            ...
            y_fi = y if isinstance(y, numfi_tmp) else fi(y_float, s, w, w-s-np.ceil(np.log2(np.max(y_float))))
            y_float = y_fi.double
            if name == 'mul':
